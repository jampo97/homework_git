...
def filter_by_currency(transactions: list[dict], currency: str | None) -> Generator:
    ...
    if currency is None:
        raise ValueError("Укажите код валюты")
    # Возвращаем генератор транзакций с условием определенной валюты
    return (
        transaction for transaction in transactions if transaction["operationAmount"]["currency"]["code"] == currency
    )


for x in filter_by_currency(my_transactions, "USD"):
    print(x)
...
for des in transaction_descriptions(my_transactions):
    print(des)


def card_number_generator(start: str, end: str) -> Generator:
    """Функция-генератор номера карты"""
    card_number: str = "0000000000000000"

    for iteration in range(int(end) + 1 - int(start)):  # количество итераций (номеров карт)
        try:
            form_cards: str = ""
...
for card in card_number_generator("10000000000", "10000000011"):
    print(card)
